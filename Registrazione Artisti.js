...
function aggiungiNuovoCliente(d) {
  ...
  if (!db) return;
  const mapDb = getMapID(db);
  const lastRow = db.getLastRow();
  const cfCol = mapDb["ID_CF"];
  const cfList = (lastRow > 1 && cfCol) ? db.getRange(2, cfCol, lastRow - 1, 1).getValues().flat().map(c => c.toString().toUpperCase()) : [];
  if (d.cf && !cfList.includes(d.cf.toUpperCase())) { inserisciRigaDb(db, d); }
}
